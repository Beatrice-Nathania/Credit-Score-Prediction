...
with col2:
    ...
    annual_income = st.text_input("Annual Income (Raw String)", "56125.5")
    ...
    num_of_loan = st.text_input("Number of Loans (Raw String)", "2")
    type_of_loan = st.text_input("Type of Loans (Comma separated)", "Credit-Builder Loan, and Mortgage Loan")
    outstanding_debt = st.text_input("Outstanding Debt (Raw String)", "370.22")

with col3:
    st.markdown("### 📊 Credit Behavior History")
    delay_from_due_date = st.number_input("Delay from Due Date (Days)", min_value=0, value=30)
    num_of_delayed_payment = st.text_input("Number of Delayed Payments (Raw String)", "14")
